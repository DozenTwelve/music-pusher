...
folder in your browser, stage it, run an `exiftool` tag check, then `beet import`
it into your library — all while watching the import logs stream live.

It is intentionally small. One upload panel, one staging list, one audit/import
console. No database, no accounts, no media server. If you already curate your
music with beets on a home server and just want a friendlier "drop a folder and
import it" workflow from any device on your network, this is for you.
...
- 🔎 **Per-album tag audit** via `exiftool -r`.
...
artwork (`.jpg .jpeg .png .webp`), sidecars (`.cue .log .txt`). Everything else
(and system files like `.DS_Store`) is skipped and reported.
...
- **[exiftool](https://exiftool.org)** for the audit step.
...
| `POST` | `/api/audit`                | Run `exiftool -r` on an album.           |
